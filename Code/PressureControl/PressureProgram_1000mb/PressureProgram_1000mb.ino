...
//Parameters
float TestVoltage= 0.5; // The target voltage at which the solenoid valve has to swtich off.
float basalVoltage= 0.02;  // The voltage limit before stimulus is applied.
unsigned long Totaldelay=0;
int TransdSensit=1000; //indica te max value of pressure transducer. 
...
void loop() {
  ...
  if(time_now > BEGINNING_LOG){
    ...
    if(time_now > BEGINNING_PROTOCOL + Totaldelay*(frame-1) + (Totaldelay/2)){
...
//  AverageanalogVoltage=getvalues.V;
    ...
    AnalogVoltage=(AnalogVoltage * 5.0)/1024.00;
