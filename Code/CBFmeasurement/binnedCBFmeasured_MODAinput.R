########################################
##
## Title:binnedCBFmeasured_MODAinput.R
##
##########################################
##
...
## and integrates the output with the stimulus table provided. It also integrates the CBFs measured with MODA into the main table.The stimulus table has information on each experiment
## (type of sample, stimulus,etc). The pressure stimulus for each time unit is also added by reading the pressure log.
## The beating state of the ciliary band is deduced from the threshold set by R. It is an conservative measure that may
## label certain bins as not beating if no frequency above the set threshold is found.
##
## Input files:
##  1. List of pressure logs.
##  2. List of files (one file per bin) with maximum intensity values in FFT images.
##  3. List of frequencies measured with MODA for each trial for each frame.
## Output files:
##  1. List of CBF (measured by moda and by FFT images) and beat state values for each bin.
##  2. List of Closure duration for each bin.(not reliable)
## Comments:
...
for (a in seq_along(StimulusTable$Trial_ID)){
  ...
  if (length(FilesKym) > 0) {
    ...
    ###ImportingCBFsModa----
    FilesCBFMOA <- list.files(path = InputPathCBFMODA, pattern = StimulusTable[a, "Trial_ID"], recursive = TRUE)
    if (length(FilesCBFMOA) > 0) {
      CBFTableMODA <- read.table(file = paste(InputPathCBFMODA, FilesCBFMOA, sep = ""),
                 header = F,
                 na.strings = " ",
                 sep = "\t")
      ListFreq <- split(CBFTableMODA$V1, ceiling(seq_along(CBFTableMODA$V1) / StimulusTable[a, "Bin_size"]))
      CBFClosureTrial <- CBFClosureTrial %>%
        mutate(CBF_MODA = unlist(lapply(ListFreq, function(X) {
        mean(X)})))
      }
